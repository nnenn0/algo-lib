...
struct C {
  ...
  C operator-(const C& a) const { return C(real - a.real, imag - a.imag); }
  bool operator<(const C& a) const {
    if (real == a.real) return imag < a.imag;
    return real < a.real;
  }
  bool operator==(const C& a) const { return real == a.real && imag == a.imag; }
  int norm() const { return real * real + imag * imag; }
};
